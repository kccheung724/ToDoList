...
import { Plus, Trash2, Calendar, Filter, Paperclip, Image, FileText, X, User, Users, Clock, CheckCircle2, Search, AlertTriangle } from 'lucide-react'
            ...
            <div className="space-y-4">
              {isEditing ? (
                ...
                <>
                  ...
                  {/* Assigned To */}
                  {(selectedTodo.assignedTo || selectedTodo.assignedGroupNames?.length > 0 || selectedTodo.assignedGroups || selectedTodo.assignedGroup) && (
                    <div>
                      <h4 className="text-sm text-muted mb-1">Assigned To</h4>
                      <p className="text-gray-300 flex items-center gap-2">
                        <Users size={16} />
                        {selectedTodo.assignedUserName}
                        {selectedTodo.assignedUserName && selectedTodo.assignedGroupNames?.length > 0 && ' / '}
                        {selectedTodo.assignedGroupNames?.map((name, idx) => (
                          <span key={idx}>
                            {idx > 0 && ', '}{name}
                          </span>
                        ))}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[60]">
          <div className="bg-surface rounded-xl p-6 border-2 border-yellow-500 max-w-md w-full animate-bounce-in animate-flash-border shadow-2xl shadow-yellow-500/20">
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => setShowDueTodayPopup(false)}
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-yellow-500 hover:bg-yellow-400 text-black rounded-lg transition-colors font-semibold"
              >
                View All
              </button>
            </div>
